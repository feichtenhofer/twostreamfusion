...
{
  vl::Error error ;
  vl::Device deviceType = a.getDeviceType() ;
  vl::Type dataType = a.getDataType() ;
  switch (deviceType) {
    default:
      assert(false) ;
                 ...
                 int nin, mxArray const *in[])
{
  ...
  if (verbosity) {
    mexPrintf("vl_accumulate: %s %s\n",
              inplace?"inplace":"not inplace",
              (a.getDeviceType() == vl::GPU)?"GPU":"CPU") ;
    vl::print("vl_accumulate: A: ", a) ;
    vl::print("vl_accumulate: B: ", b) ;
  }

  vl::Error error = vl::accumulate(context, output, alpha, a, beta, b);
